...
namespace Bober {
  ...
  void MarkerStyle::ExportToXML(XMLNode* node) const {
    ...
    if (Find(kColor)) node->AddAttrib(new Bober::XMLAttrib("Color", Form("%i", GetI(kColor))));
    if (Find(kStyle)) node->AddAttrib(new Bober::XMLAttrib("Style", Form("%i", GetI(kStyle))));
    //  if (Find(kLineWidth)) node->AddAttrib(new Bober::XMLAttrib("LineWidth", Form("%i", GetI(kLineWidth))));
    if (Find(kSize)) node->AddAttrib(new Bober::XMLAttrib("Size", Form("%i", GetI(kSize))));
  }
