...
namespace Bober {
  ...
  void AxisStyle::ExportToXML(XMLNode* node) const {
    if (Find(kTitleOffset)) node->AddAttrib(new Bober::XMLAttrib("TitleOffset", Form("%4.4f", GetF(kTitleOffset))));
    if (Find(kLabelOffset)) node->AddAttrib(new Bober::XMLAttrib("LabelOffset", Form("%4.4f", GetF(kLabelOffset))));
    if (Find(kTitleSize)) node->AddAttrib(new Bober::XMLAttrib("TitleSize", Form("%4.4f", GetF(kTitleSize))));
    if (Find(kLabelSize)) node->AddAttrib(new Bober::XMLAttrib("LabelSize", Form("%4.4f", GetF(kLabelSize))));
    if (Find(kTickLength)) node->AddAttrib(new Bober::XMLAttrib("TickLength", Form("%4.4f", GetF(kTickLength))));
    if (Find(kNdivisions)) node->AddAttrib(new Bober::XMLAttrib("Ndivisions", Form("%i", GetI(kNdivisions))));
    if (Find(kAxisColor)) node->AddAttrib(new Bober::XMLAttrib("AxisColor", Form("%i", GetI(kAxisColor))));
    if (Find(kLabelColor)) node->AddAttrib(new Bober::XMLAttrib("LabelColor", Form("%i", GetI(kLabelColor))));
